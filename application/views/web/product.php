...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="center" height="198px" style="background:#cccccc url(Images/topbannerbg.jpg) no-repeat center top;"><img src="<?php echo $this->config->item('base_url');?>/public/index/Picture/banner_pro.jpg" width="1001" height="198"></td>
  </tr>
</table>
<div id="container">
...
<DIV id="right_menu" style="BACKGROUND: url(Images/partat2.gif) no-repeat top left;">
    <H3><a href="list.asp?bc=2"><B><?php echo $navName?></B></a></H3>
    <UL style="padding:0 0">
      <?php if(!empty($menulist)) foreach($menulist as $k=>$v){?>
            <li style="color:#000"><?php echo !empty($v['typeName']) ? $v['typeName'] : ''?></li>
            <?php if(!empty($v['productlist'])) foreach($v['productlist'] as $key=>$val){?>
            <li><a href='<?php echo $this->config->item('base_url').'/Index/productcontents/'.$val['productId'];?>'>&nbsp;&nbsp;<?php echo !empty($val['title']) ? $val['title'] : ''?></font></a></li>
            <?php }?>
      <?php }?>
    </UL>
    <br style="clear:both;" />
    ...
    <tr valign='top'>
        ...
        <?php if(!empty($product)) foreach($product as $key=>$val){?>
        ...
        <tr>
        	...
        	<a href='<?php echo $this->config->item('base_url').'/Index/productcontents/'.$val['productId'];?>'><img src='<?php echo $this->config->item('img_url').$val['imageName'];?>' width='260' height='180' border='0'>
            </a>
            </td>
            <td width='400' height='30'>
            <a href='<?php echo $this->config->item('base_url').'/Index/productcontents/'.$val['productId'];?>' class='nav2'><?php echo $val['title']?></a>
            </td>
            <td width='20' rowspan='2'>
            <a href='<?php echo $this->config->item('base_url').'/Index/productcontents/'.$val['productId'];?>'>
