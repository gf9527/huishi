...
<DIV id="right_menu" style="BACKGROUND: url(<?php echo $this->config->item('base_url');?>/public/index/Images/partat2.gif) no-repeat top left;">
    <H3><a href="list.asp?bc=2"><B>产品中心</B></a></H3>
    ...
    <?php if(!empty($menulist)) foreach($menulist as $k=>$v){?>
            ...
            <li><a href='<?php echo $this->config->item('base_url').'/Index/productcontents/'.$val['productId'];?>'><?php if($val['productId'] == $info['productId']){echo '<font class="leftsc">';}?>&nbsp;&nbsp;<?php echo !empty($val['title']) ? $val['title'] : ''?></font></a></li>
            <?php }?>
      <?php }?>
    </UL>
    <br style="clear:both;" />
                  ...
                  <td class="t12black" id="fontzoom" style="line-height:24px; padding-top:20px">
                    ...
                    <div id="lib_Tab1">
                      <div class="lib_Menubox lib_tabborder">
                        <ul>
                          <li id="one1" onClick="setTab('one',1,4)" class="hover"><span>产品概述</span></li>
                          <li id="one2" onClick="setTab('one',2,4)" ><span>规格参数</span></li> 
                        </ul>
                      </div>
